...
class KeyChainManager {
    ...
    @available(*, deprecated, message: "KeyChainManager.defaultKeyChainManager is deprecated since version 2.2.1, use KeyChainManager.standard instead")
    public static let defaultKeyChainManager = KeyChainManager.standard
    
    /// Default keychain manager access
    public static let standard = KeyChainManager()
    
    /// ServiceName is used for the kSecAttrService property to uniquely identify this keychain accessor. If no service name is specified, KeyChainManager will default to using the bundleIdentifier.
    private (set) public var serviceName: String
    
    /// AccessGroup is used for the kSecAttrAccessGroup property to identify which Keychain Access Group this entry belongs to. This allows you to use the KeyChainManager with shared keychain access between different applications.
    private (set) public var accessGroup: String?
    
    private static let defaultServiceName: String = {
        return Bundle.main.bundleIdentifier ?? "SwiftKeyChainManager"
    }()
    ...
    /// Create a custom instance of KeyChainManager with a custom Service Name and optional custom access group.
    ///
    ...
    /// Remove all keychain data added through KeyChainManager. This will only delete items matching the currnt ServiceName and AccessGroup if one is set.
    ...
    ///
    /// - Warning: This may remove custom keychain entries you did not add via SwiftKeyChainManager.
    ///
