...
public class QRCameraPreview: UIView {
    ...
    var onDraw: QRCodeScanner.OnDraw?
    var onFound: QRCodeScanner.OnFound?
    var mockBarCode: QRCodeData?
    ...
    var lastScannedBarcode: QRCodeData?
    ...
    @objc func onClick() {
        foundBarcode(mockBarCode ?? QRCodeData(value: "Mock Value", type: .qr))
    }
    ...
    public func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if let metadataObject = metadataObjects.first {
            guard let readableObject = metadataObject as? AVMetadataMachineReadableCodeObject else { return }
                ...
                let frame = QRCodeFrame(corners: corners, cameraPreviewView: self)
                onDraw?(frame)
            #endif

            if let stringValue = readableObject.stringValue {
                let barcode = QRCodeData(value: stringValue, type: readableObject.type)
                foundBarcode(barcode)
            }
        }
    }

    func foundBarcode(_ barcode: QRCodeData) {
        let now = Date()
